/*! WindowTabBar — Chrome 风格浏览器标签组件（FlutterLane 内部控件）
 *
...
class TabData {
  ...
  /// 业务唯一标识（如 course_id）；null 表示无业务绑定
  final String? businessId;

  /// 是否允许关闭；false 时不渲染关闭按钮，close() 调用被拦截
  final bool closable;
...
/// 溢出处理模式：
/// [fixedCount] — 最多显示固定数量标签，超出部分进溢出菜单（不受视口宽度影响）
/// [viewportWidth] — 传统模式，按视口宽度动态计算可见数量
enum TabOverflowMode { fixedCount, viewportWidth }
...
class TabBarController extends ChangeNotifier {
  ...
  /// [overflowMode] 溢出处理模式；[maxVisibleFixedCount] fixedCount 模式下最多可见数
  TabBarController({
    List<TabData> initialTabs = const [],
    this.onChange,
    this.newTabLabel = '新标签页',
    this.overflowMode = TabOverflowMode.viewportWidth,
    this.maxVisibleFixedCount = 0,
  }) : _tabs = <TabData>[] {
  ...
  /// 溢出处理模式
  final TabOverflowMode overflowMode;

  /// fixedCount 模式下最多可见标签数；0 = 不限制（退化为 viewportWidth）
  final int maxVisibleFixedCount;
  ...
  /// 若标签 [closable] 为 false，则拦截关闭请求（不执行任何操作）。
  void close(String id) {
    final idx = indexOf(id);
    if (idx < 0) return;
    final tab = _tabs[idx];
    if (!tab.closable) return; // 拦截不可关闭的标签
  ...
  void activate(String id) {
    ...
    notifyListeners();
    _emit(
        TabBarChangeEvent(type: TabChangeType.activate, id: id, data: get(id)));
  }

  /// 更新标题 / 地址 / 业务标识 / 可关闭性
  void update(String id,
      {String? title, String? url, String? businessId, bool? closable}) {
    final idx = indexOf(id);
    if (idx < 0) return;
    final old = _tabs[idx];
    _tabs[idx] = TabData(
      ...
      url: url ?? old.url,
      color: old.color,
      businessId: businessId ?? old.businessId,
      closable: closable ?? old.closable,
    );
    notifyListeners();
    _emit(TabBarChangeEvent(
        type: TabChangeType.update, id: id, data: _tabs[idx]));
  }

  /// 用外部权威数据整体替换标签集（业务集成方状态来自后端时使用）。
  /// 保持原激活项不变；若原激活项已不存在，则激活新列表首个标签。
  /// 传入 [activateId] 时静默激活该标签（不触发 activate 事件，防止与上层状态回流）。
  /// 仅通知重绘，不触发 add/close 业务事件，避免与上层状态回流形成循环。
  void replaceAll(List<TabData> next, {String? activateId}) {
    final resolved = next.map(_resolve).toList();
    _tabs
      ..clear()
      ..addAll(resolved);
    if (activateId != null && get(activateId) != null) {
      _activeId = activateId;
    } else if (_activeId != null && get(_activeId!) == null) {
      _activeId = resolved.isEmpty ? null : resolved.first.id;
    }
    notifyListeners();
  }
...
class WindowTabBar extends StatefulWidget {
  const WindowTabBar({
    super.key,
  ...
  /// 用 ValueKey 而非 GlobalKey：多条 WindowTabBar 并存时不会产生重复 Key 冲突。
  static const Key overflowButtonKey = ValueKey('window_tab_bar_overflow');
  ...
  /// 新增标签请求（+ 按钮 / 右键"新建"）。非空时**拦截底层默认 add 逻辑**，
  /// 由使用方（如打开业务课程选择弹窗）自行处理；空时保持上游默认 add 行为。
  ...
  /// 新增标签请求拦截回调；非空时 + 按钮 / 右键"新建"不再执行底层 add，改为回调
  final VoidCallback? onNewTabRequested;

  @override
  State<WindowTabBar> createState() => _WindowTabBarState();
}
...
class _WindowTabBarState extends State<WindowTabBar> {
  ...
  void initState() {
    ...
    if (outer != null) {
      ...
      widget.controller.onChange = prev == null
          ? outer
          : (e) {
              prev(e);
              outer(e);
  ...
  ///
  /// [fixedCount] 模式下，[viewportInner] 被忽略，直接使用
  /// controller.maxVisibleFixedCount 作为可见数量；至少保留 1 个。
  _OverflowLayout _overflowLayout(double viewportInner, int count) {
    final c = widget.controller;
    int visibleCount;
    double tabWidth;
    if (c.overflowMode == TabOverflowMode.fixedCount &&
        c.maxVisibleFixedCount > 0) {
      visibleCount = math.max(1, math.min(count, c.maxVisibleFixedCount));
      tabWidth = (viewportInner / visibleCount)
          .clamp(TabBarStyle.minTabWidth, TabBarStyle.maxTabWidth);
    } else {
      visibleCount = math.max(
          1, math.min(count, viewportInner ~/ TabBarStyle.minTabWidth));
      tabWidth = (viewportInner / visibleCount)
          .clamp(TabBarStyle.minTabWidth, TabBarStyle.maxTabWidth);
    }
      ...
      child: CompositedTransformFollower(
        ...
        child: TapRegion(
          ...
          child: _TabContextMenu(
            style: style,
            onNewTab: () => run(widget.onNewTabRequested ??
                () =>
                    c.add(TabData(title: widget.newTabLabel ?? c.newTabLabel))),
            onClose: (id == null || !c.tabs[idx].closable)
                ? null
                : () => run(() => c.close(id)),
            onCloseLeft: (id == null || idx == 0)
                ? null
                : () {
                    // 左侧有不可关闭的 tab 时禁用（防止误关保护 tab）
                    final hasUnclosable =
                        c.tabs.sublist(0, idx).any((t) => !t.closable);
                    if (hasUnclosable) return;
                    run(() => c.closeLeft(id));
                  },
            onCloseOthers: (id == null || c.count < 2)
                ? null
                : () {
                    final others = c.tabs.where((t) => t.id != id);
                    if (others.any((t) => !t.closable)) return;
            ...
            onCloseRight: (id == null || idx == c.count - 1)
                ...
                : () {
                    final hasUnclosable =
                        c.tabs.sublist(idx + 1).any((t) => !t.closable);
                    if (hasUnclosable) return;
                    run(() => c.closeRight(id));
                  },
          ),
            ...
            child: OverlayPortal(
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  // 溢出判定：fixedCount 模式按固定数量阈值；viewportWidth 模式按最小宽计算
                  final fixedMax = c.overflowMode == TabOverflowMode.fixedCount
                      ? (c.maxVisibleFixedCount > 0
                          ? c.maxVisibleFixedCount
                          : count)
                      : 0;
                  final over = fixedMax > 0
                      ? count > fixedMax
                      : (count > 0 &&
                          count * TabBarStyle.minTabWidth > base - ear);
                  ...
                  return Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        if (over)
                          _OverflowButton(
                            key: WindowTabBar.overflowButtonKey,
                            controller: c,
                            style: style,
                        ...
                        _NewTabButton(
                          ...
                          onTap: widget.onNewTabRequested ??
                              () => c.add(TabData(
                                  title: widget.newTabLabel ?? c.newTabLabel)),
                        ),
...
class _TabsAreaState extends State<_TabsArea> with TickerProviderStateMixin {
  ...
  void _onTick(Duration elapsed) {
    ...
    for (var i = 0; i < c.count; i++) {
      ...
      settled &= _approach(_closeAmt, i,
          (i == activeIdx || i == _hoverIndex) ? 1.0 : 0.0, decay);
    }
  ...
  bool _inClose(Offset p, int i) {
    ...
    final x0 =
        rel * widget.tabWidth + widget.tabWidth - 8 - TabBarStyle.closeSize;
    final top = (TabBarStyle.tabHeight - TabBarStyle.closeSize) / 2; // 垂直居中
    // closable=false 的 tab 不响应关闭命中
    if (i >= 0 &&
        i < widget.controller.count &&
        !widget.controller.tabs[i].closable) return false;
    return p.dx >= x0 &&
          ...
          child: Listener(
            ...
            onPointerDown: (e) {
              if (e.buttons & kSecondaryButton != 0) {
                ...
                if (i != null && c.tabs[i].closable) c.close(c.tabs[i].id!);
              }
            },
            child: GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTapUp: (d) {
                ...
                if (i == null) return;
                if (_inClose(p, i)) {
                  // 不可关闭标签：拦截关闭，震动反馈（未来可扩展）
                  if (!c.tabs[i].closable) return;
                  c.close(c.tabs[i].id!);
                } else {
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    if (key == LogicalKeyboardKey.contextMenu ||
      ...
      final rel =
          (_focusIndex - widget.visibleStart).clamp(0, widget.visibleCount - 1);
      final box = context.findRenderObject()! as RenderBox;
      widget.onContextMenu?.call(
        _focusIndex,
        box.localToGlobal(
        ...
        child: Padding(
          ...
          child: SizedBox(
            ...
            child: CustomPaint(
              ...
              // 同样盖在悬停底之上
              painter:
                  _NewTabHoverPainter(style: widget.style, hovered: _hover),
              child: widget.icon ??
                  CustomPaint(
      ...
      child: CompositedTransformFollower(
        ...
        child: TapRegion(
          ...
          child: _OverflowMenu(
            ...
            // closable=false 的条目不渲染关闭按钮（onClose 为 null 时隐藏）
            onClose: (id) {
              if (!(widget.controller.get(id)?.closable ?? true)) return;
                    ...
                    children: [
                      for (final t in tabs)
                        _OverflowMenuItem(
                          ...
                          // closable=false 的条目隐藏关闭按钮
                          onClose: (onClose != null && t.closable)
                              ? () => onClose!(t.id!)
                              : null,
                        ),
                    ],
  ...
  void paint(Canvas canvas, Size size) {
    ...
    switch (icon) {
      ...
      case _MenuIcon.close:
        canvas.drawLine(
            c + const Offset(-4.5, -4.5), c + const Offset(4.5, 4.5), p);
        canvas.drawLine(
            c + const Offset(4.5, -4.5), c + const Offset(-4.5, 4.5), p);
    }
  }
...
class _TabStripPainter extends CustomPainter {
  ...
  void _paintTab(Canvas canvas, int index, TabData tab) {
    ...
    final favColor =
        tab.color ?? kDefaultPalette[index % kDefaultPalette.length];
    canvas.drawRRect(
    ...
    // closable=false 时不渲染关闭按钮，标题区宽多出 24px（16 按钮 + 8 间距）
    final closeBtnW = tab.closable ? 24 : 0;
    final labelW = w - 32 - closeBtnW; // 8 + 16(fav) + 8 + close/8
    if (labelW > 12) {
      final tp = _titleOf(tab, isActive, labelW);
      tp.paint(canvas, Offset(32, (_h - tp.height) / 2));
    }

    // ---- 7) 关闭按钮：16×16；激活常显 / 悬停 150ms 渐显；closable=false 时不渲染 ----
    final vis = (isActive && tab.closable) ? 1.0 : 0.0;
    if (vis > 0.01) {
  ...
  void _paintLetter(Canvas canvas, Rect rect, String title, Color bg) {
    final color =
        _isLightColor(bg) ? const Color(0xFF1A1A1F) : const Color(0xFFFFFFFF);
